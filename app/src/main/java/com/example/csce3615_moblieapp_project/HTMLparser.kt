...
//    }
    @RequiresApi(Build.VERSION_CODES.O)
    fun retrieveMenu(cafeChoice: String? /*, dayOffset: Int*/): Vector<DiningHallMenuItem> {
        ...
        val menuList = Vector<DiningHallMenuItem>()
        var numChoice = 0
        when (cafeChoice) {
            "BRUCE" -> numChoice = 0
            "CHAMPS" -> numChoice = 1
        ...
        try {
            ...
            while (myReader.hasNextLine()) {
                ...
                if (data.contains("[] -- ")) //Determine category
                {
                    ...
                    do {
                        val temp = DiningHallMenuItem(
                            id = data.substring(0, 8),
                            name = data.substring(9, data.indexOf('|')),
                            allergens = data.substring(9, data.indexOf('|'))
